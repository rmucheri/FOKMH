const members = [
    {
        img: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png",
        header: "Thomas Gondo ",
        text: "Thomas Gondo is a Licensed Land Surveyor in Ontario Canada and the President of Lejan Land Surveying Inc, a Licensed Practice also in the province of Ontario. He has consistently been involded in efforts to fundraise for Karanda Hospital."
    },
    {
        img: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png",
        header:"Victoria S Mamvura",
      text: `"Victoria Siphiwe Mamvura holds a Masters, Post Graduate Diploma in HIV and AIDS, and Health Communication: Research, Monitoring & Evaluation of HIV & AIDS Projects, Developing Workplace Wellness Programs and Strategic Management in the field of HIV
& AIDS. She has worked in different portfolios in Zimbabwe and Namibia working with organizations like University of Namibia, Bank Windhoek Namibia, Broadcasting Authority of Zimbabwe, Namibia Student Financial Assistance Fund amongst other organizations."`
    },
    {
        img: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png",
        header:"Blessing Mucherera",
        text:"Blessing is a PhD researcher in Human Geography (Water Cultures), at the University of Hull, United Kingdom. With over 20 years of experience working with various charity organizations across Africa, the United Kingdom, and North America, Blessing brings a wealth of knowledge and expertise to our board. He is currently Chairperson, Bindura University of Science Education Member, Yorkshire and Humber Climate Commission (February 2022 - Present) Member, Hull Afro-Caribbean Association. Blessing's extensive experience in academia and his active involvement in climate and community organizations make him an invaluable asset to our team. His leadership and commitment to environmental and social causes align with our mission to create lasting positive change"
    },
    {
        img: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png",
        header:"Lindiwe Nyamukondiwa",
        text:"Lindiwe is a registered nurse licensed in Alberta, Canada, holding a master’s degree in nursing. She brings twenty years of experience in geriatric nursing and is currently serving as a Clinical Manager at an organization in Calgary. Lindiwe is enthusiastic about joining the Friends of Karanda Mission Hospital board and is particularly eager to contribute to initiatives involving volunteer nurses through the 'Nurses Without Borders' program to support Karanda Hospital."
        
    },
    {
        img: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png",
        header: "Mr. Sanyaduru Mutepfa",
        text: "Mr. Sanyaduru Mutepfa is a seasoned professional with extensive experience in general management, encompassing operations, finance, IT, marketing, legal, and human resources. He began his career in 1980 with Zimbabwe’s Ministry of Manpower, where he developed expertise in manpower planning and also served as a part-time lecturer in management. Mr. Mutepfa has significant governance experience, having served on the board of the Small to Medium Enterprises Development Corporation, where he chaired the Human Resources Subcommittee. He has cultivated a global network of contacts and is passionate about environmental issues, particularly climate change and renewable energy. He is also committed to supporting the less privileged, including those with health and physical challenges. He holds a B.A. in Sociology and Geography, a Diploma in Education, a Post Graduate Diploma in Population Studies, a Master of Business Leadership (MBL), and a Certificate in HIV/AIDS Care and Counselling. Mr. Mutepfa has held several senior managerial positions, including General Manager and Human Resources Director at Delta Distribution, Group Manpower Development Manager at Zimbabwe Sun Hotels, and Training Manager at Cold Storage Commission. His broad range of collaborative experience and corporate interactions in Zimbabwe will be valuable to FOKMH."
    },
    {
        img: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png",
        header: "Mr. Patrick Hokonya",
        text: "Mr. P Hokonya is a highly motivated and experienced professional with a strong background in management, leadership, and customer service, gained in both private and nonprofit sectors. He currently works as a Customer Experience Associate at Scotiabank, where he excels in customer engagement, sales, and financial advisory roles. Prior to this, He co-founded Lawsure Financial Services, a legal aid startup, where he led business development, sales strategies, and successfully expanded the company's reach across major cities. From 1999 to 2015, he held various positions at Econet Wireless, culminating in the role of Branch Manager. In this capacity, he managed operations, staff, and customer service across multiple sites, significantly growing the subscriber base. He also served as an Account Manager and Customer Care Supervisor, roles in which he developed high-value customer relationships and implemented effective customer service strategies. Mr. P Hokonya core competencies include sales, account management, business development, and relationship management. He holds a Bachelor of Commerce in Marketing Honours from Midlands State University and have extensive volunteer experience, including serving as a Board Member and Trustee for Econet Wireless Pension Fund and a National Board Member for the Zimbabwe Football Association."
    },
    {
        img: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png",
        header: "Mrs. Zanele Olujonwo",
        text: "Zanele Olujonwo works as a Program Coordinator for a non-profit organization in Calgary, Alberta, Canada. She holds a certificate in occupational health and safety from the University of Victoria, an MBA from Southern New Hampshire University and a Master's in Health Care Administration from Rivier University in Nashua, New Hampshire. She is also a licensed Nursing Home Administrator in the State of Massachusetts. Zanele believes in the profound value of serving others, echoing Lewis Carroll's sentiment: 'One of the deep secrets of life is that all that is worth doing is what we do for others.'"
    },
    {
        img: "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png",
        header: "Dr Causemore Zengeni",
        text: "Dr. C. Zengeni graduated from the University of Zimbabwe Medical School in 2005. After completing an 18-month tenure at Harare Hospital, he moved to the United Kingdom, where he undertook training in Family Medicine. He is currently a partner at Ash Tree House Surgery in Kirkham. Dr. Zengeni holds the Membership of the Royal Colleges of Physicians (MRCP UK), a specialty certificate in Diabetes and Endocrinology, and a Diploma in the Management of Diabetes in Primary Care. He is a member of the Royal College of Physicians of Edinburgh and the Society for Endocrinology."
    },
    {
        img: "https://media.licdn.com/dms/image/D5603AQHSvrsbRqQFtA/profile-displayphoto-shrink_200_200/0/1707094646587?e=1725494400&v=beta&t=cbHD_Rmcdksq4lw3571KZZZ5behFE3j3PQ0s5DwuBMI",
        header: "Mr. Mwandibhuya K Mutepfa",
        text: "Mr. Mwandibhuya Kennedy Mutepfa is the Founder and CEO of several private enterprises in Calgary, Canada. Prior to his entrepreneurial ventures, he held significant positions in the industrial sector, serving as a Senior Buyer at Lancashire Steel and later as Retail Manager and General Manager at Ziscosteel Distribution Center in Kwekwe. His leadership extended to the realm of sports, where he held multiple national soccer leadership roles, demonstrating his passion for the sport and administrative acumen. He also currently serve as a board member with the Calgary United Soccer Association (CUSA) Mr. Mutepfa is a proponent of good corporate governance, strategic planning, and effective policy formulation and implementation. He has earned several certifications in stores and purchasing management, soccer administration, entrepreneurship and a Diploma in business and digital marketing management, He also holds an Executive Master of Business Administration (EMBA) with a focus on professional selling and leadership from the university of Fredericton, New Brunswick, Canada. As one of the co-founders of Friends of Karanda Mission Hospital (FOKMH), Mr. Mutepfa believes in the importance of giving back to the community. He advocates for sharing resources with those in need, emphasizing the positive impact such contributions can have on the less privileged."
    },
   
   
]

export default members